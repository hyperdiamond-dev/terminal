...
interface SubmoduleQuestionnaireProps {
  moduleName: string;
...
export default function SubmoduleQuestionnaire({
  ...
  const nextSubmodule = useSignal<string | null>(null);
  const moduleCompleted = useSignal(false);
  ...
  const completeSubmodule = async () => {
    ...
    try {
      ...
      if (data.unlocked_submodules && data.unlocked_submodules.length > 0) {
        nextSubmodule.value = data.unlocked_submodules[0].name;
      }
      moduleCompleted.value = data.module_completed || false;
  ...
  // Review mode (completed submodule)
  if (isCompleted) {
    return (
      <div class="my-8">
      ...
      <div class="my-8 text-center">
        <div class="border-2 border-vhs-gray-dark bg-decay-smoke/30 p-8 my-6">
          ...
          <button
            ...
            `}
          >
            {isSubmitting.value ? "&gt; STARTING..." : "&gt; BEGIN SECTION"}
          </button>
        </div>
